...
from service.modularized_supervised import main_supervised
from service.modularized_unsupervised import main_unsupervised
...
def task_insertring(self, data: dict):
    try:
        ...
        export_loc = f"{settings.EXPORT_DIR}/Insert_Ring/{date_today}/{self.request.id}"
...
@celery_app.task(name="tasks.heavy.supervised_ring", bind=True, max_retries=3, default_retry_delay=60)
def task_supervised(self, data: dict):
    try:
        print(f"Celery Fiberization | Supervised Task Started | Task ID: {self.request.id}")
        parsed_data = loads(data)
        site_path = parsed_data.get("site_path")
        fiber_path = parsed_data.get("fiber_path")
        ...
        if DOCKER:
            ...
            if fiber_path:
                if "/mnt/" not in fiber_path:
                    fiber_path = fiber_path.replace("uploads", "/mnt/uploads").replace("\\", "/")

        # LOAD DATA
        site_data = gpd.read_parquet(site_path)
        fo_expand = gpd.read_parquet(fiber_path) if fiber_path else None

        date_today = datetime.now().strftime("%Y%m%d")
        export_loc = f"{EXPORT_DIR}/Supervised Ring/{date_today}/{self.request.id}"
        os.makedirs(export_loc, exist_ok=True)

        self.update_state(state="PROGRESS", meta={"status": "Processing supervised fiberization data"})
        ...
        self.update_state(
            state="SUCCESS",
            meta={"status": "Supervised fiberization data processed successfully", "result": result, "zip_file": zip_filepath},
        )
        print(f"Celery | Supervised Task Completed | Task ID: {self.request.id}")

        # CLEAN UP TEMP FILES
        try:
            if os.path.exists(site_path):
                os.remove(site_path)
            if fiber_path and os.path.exists(fiber_path):
                os.remove(fiber_path)
        except Exception as cleanup_error:
            print(f"Error during cleanup of temporary files: {str(cleanup_error)}")
...
@celery_app.task(name="tasks.heavy.unsupervised_ring", bind=True, max_retries=3, default_retry_delay=60)
def task_unsupervised(self, data: dict):
    try:
        print(f"Celery Fiberization | Unsupervised Task Started | Task ID: {self.request.id}")
        parsed_data = loads(data)
        hubs_path = parsed_data.get("hub_path")
        site_path = parsed_data.get("site_path")
        fiber_path = parsed_data.get("fiber_path")
        member_expectation = parsed_data.get("member_expectation")
        member_tolerance = parsed_data.get("member_tolerance")
        method = parsed_data.get("method", "unsupervised")
        area_col = parsed_data.get("area_col", 'region')
        cluster_col = parsed_data.get("cluster_col", 'ring_name')
        ...
        date_today = datetime.now().strftime("%Y%m%d")
        export_loc = f"{EXPORT_DIR}/Unsupervised Ring/{date_today}/{self.request.id}"
        os.makedirs(export_loc, exist_ok=True)

        self.update_state(state="PROGRESS", meta={"status": "Processing unsupervised fiberization data"})
        start_time = time()
        ...
        self.update_state(
            state="SUCCESS",
            meta={"status": "Unsupervised fiberization data processed successfully", "result": result, "zip_file": zip_filepath},
        )
        print(f"Celery | Supervised Task Completed | Task ID: {self.request.id}")

        elapsed_time = end_time - start_time
        print(f"ℹ️ Elapsed Time for Unsupervised: {round(elapsed_time/60, 2)} minute")
...
# TASK FOR FIX ROUTE
@celery_app.task(name="tasks.heavy.fixroute", bind=True, max_retries=3, default_retry_delay=60)
def task_fixroute(self, data: dict):
    try:
        ...
        route_type = parsed_data.get("route_type", 'merged')
        ...
        date_today = datetime.now().strftime("%Y%m%d")
        export_loc = f"{EXPORT_DIR}/New_Ring/Fix Route/{date_today}/{self.request.id}"
        os.makedirs(export_loc, exist_ok=True)
        ...
        print(f"Celery | Fix Route Task Completed | Task ID: {self.request.id}")
