...
from tasks.fiberization_celery import task_insertring, task_supervised, task_unsupervised, task_fixroute
...
# CHECK TASK STATUS
# =============================
@router.get("/tasks/status/{task_id}", tags=["Fiberization"])
...
# =============================
@router.post("/identify_insert", tags=["Fiberization"])
...
# INSERT RING
# =============================
@router.post("/insert_ring", tags=["Fiberization"])
async def insert_ring(
    ...
    """
    # Read previous fiber file
    upload_insert_dir = os.path.join(UPLOAD_DIR, "Insert Ring")
...
# SUPERVISED
@router.post("/new_ring/supervised", tags=["Fiberization"])
async def supervised_ring(
    excel_file: UploadFile = File(
        None, description="Excel file containing ring data."
    ),
    fiber_route: Optional[UploadFile|str] = File(
        None, description="GPKG, Parquet, or Shapefile containing fiber route data."
    ),
):
    """
    ...
    **Note:**
    ...
    upload_newring_dir = os.path.join(UPLOAD_DIR, "Supervised Ring")
    ...
    # Read fiber route file if provided
    fiber_gdf = None
    if fiber_route is not None:
        try:
            print(f"ℹ️ Reading fiber route file: {fiber_route}")
            suffix = os.path.splitext(fiber_route.filename)[1].lower()
            with tempfile.NamedTemporaryFile(delete=False, suffix=suffix) as tmp:
                tmp.write(fiber_route.file.read())
                tmp_path = tmp.name

            if suffix in ['.gpkg', 'geojson', '.shp']:
                fiber_gdf = gpd.read_file(tmp_path)
                print(f"📥 Reading fiber route file: {fiber_route.filename}")
            if suffix == '.parquet':
                fiber_gdf = gpd.read_parquet(tmp_path)
                print(f"📥 Reading fiber route file: {fiber_route.filename}")
            else:
                return {"error": "Unsupported fiber route file format. Supported formats are GPKG, Parquet, and Shapefile."}
        except Exception as e:
            return {"error": f"Failed to read fiber route file: {str(e)}"}
        finally:
    ...
    if fiber_gdf is not None:
        temp_fiber_path = os.path.join(upload_newring_dir, f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_fiber_data_{uuid4().hex}.parquet")
        fiber_gdf.to_parquet(temp_fiber_path, index=False)
        print(f"📥 Temporary fiber data saved to: {temp_fiber_path}")
    ...
    try:
        data = {
            ...
            "fiber_path": temp_fiber_path if fiber_gdf is not None else None,
            "method": method
        }
        data = dumps(data, default=str)
        print("🚀 Initiating supervised fiberization task...")
...
# SUPERVISED
@router.post("/new_ring/unsupervised", tags=["Fiberization"])
async def create_unsupervised(
    excel_file: UploadFile = File(
    ...
    member_tolerance: int = Form(0, description="Member tolerance for exceed."),
    ...
    **Note:**
    ...
    upload_newring_dir = os.path.join(UPLOAD_DIR, "Unsupervised Ring")
    os.makedirs(upload_newring_dir, exist_ok=True)
    method = "unsupervised"
    ...
    try:
        ...
        data = dumps(data, default=str)
        print("🚀 Initiating unsupervised fiberization task...")
...
# FIX ROUTE
@router.post("/new_ring/fixroute", tags=["Fiberization"])
async def create_fixroute(
    excel_file: UploadFile = File(None, description="Excel file containing fix route template."),
    route_type: str = Form("merged", description="Route type to use: 'merged' or 'classified'"),
    program_name: str = Form(None, description="Program name if not defined")
):
    """
    ...
    **Note:**
    ...
    # Validate method
    if route_type not in ['merged', 'classified']:
        return {"error": "Method must be either 'merged' or 'classified'."}
    ...
    try:
        ...
        data = dumps(data, default=str)
        print("🚀 Initiating Fix Route task")
...
@router.post("/update_intersite", tags=["Fiberization"])
