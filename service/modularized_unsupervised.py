...
from modules.utils import create_topology, route_path, spof_detection, dropwire_connection
from modules.table import sanitize_header, detect_week, excel_styler
from modules.validation import input_unsupervised, input_newring
from modules.h3_route import identify_hexagon, retrieve_roads, build_graph
from modules.clustering import dm_cluster, main_clustering
...
def distance_mapping(cluster_sites, G=None, weight="weight"):
    ...
    for future in tqdm(
        ...
        try:
            ...
            path_length = future.result() if future.result() is not None else 99999999999
            mapping[(id_i, id_j)] = path_length
            mapping[(id_j, id_i)] = path_length
        except Exception as e:
            # print(f"Error calculating path between {i} and {j}: {e}")
            mapping[(id_i, id_j)] = 99999999999
            mapping[(id_j, id_i)] = 99999999999
    return mapping
...
def single_fohub(
    ...
    algorithms = {
        "Threshold TSP": lambda g: nx.approximation.threshold_accepting_tsp(g, init_cycle=init_cycle, weight="weight", max_iterations=10, seed=42, source=hub_node),
        # "Simulated Annealing": lambda g: nx.approximation.simulated_annealing_tsp(g, init_cycle=init_cycle, weight="weight", max_iterations=10, seed=42, source=hub_node),
        ...
        try:
            ...
            if sitepath and len(sitepath) > 1:
                ...
                for idx, node in enumerate(sitepath[:-1]):
                    ...
                    end_node = cluster_site.loc[cluster_site["site_id"] == sitepath[idx + 1], "nearest_node"].values[0]
                    try:
                        ...
                        print(f"Cluster {cluster:20} | {start_site['site_id']:>15} to {end_site['site_id']:>15} | Length: {path_length:8.2f} meters")
                        ...
                        print(
                            f"No path found for {algo_name} | Cluster {cluster} | Start: {start_site['SiteId TBG']} | End: {end_site['SiteId TBG']}"
                        )
                        continue
                    except Exception as e:
                        print(
                            f"❌ Error processing path for {algo_name} | Cluster {cluster}: {e}"
                        )
                        continue
                ...
                for node in sitepath:
                    ...
                    if site["site_id"] not in [
                        point["site_id"] for point in points_data
                    ]:
                        points_data.append(
                            {
                                "site_id": site["site_id"],
                                "site_name": (
                                    site["site_name"] if "site_name" in site else None
                                ),
                                "site_type": (
                                    site["site_type"] if "site_type" in site else None
                                ),
                                area_col: area,
                                "algo": algo_name,
                                "ring_name": cluster,
                ...
                print(
                    f"⚠️ No valid path found for {algo_name} in cluster {cluster}. Skipping..."
                )
                continue
        except Exception as e:
...
def multi_fohub(
    ...
    sitelist = cluster_site[cluster_site["site_type"] != "FO Hub"].copy()
    fo_hub = cluster_site[cluster_site["site_type"] == "FO Hub"].copy()
...
def cluster_ring(cluster_args):
    ...
    print(
        f"🛟 Processing Cluster {cluster} | Area: {area} | Total Site: {len(cluster_site):,}"
    )
    ...
    G = build_graph(roads, graph_type="full_weighted", ref_fo=ref_fo, cable_cost=cable_cost)
...
def parallel_ring(
    ...
    **kwargs,
    ...
    clusters = sorted(clustered_sites[cluster_col].dropna().unique().tolist())
    ...
    with ProcessPoolExecutor(max_workers=1) as executor:
        ...
        if task_celery:
            task_celery.update_state(
                state="PROGRESS",
                meta={
                    "status": f"Processing {total_futures} clusters for area {area}",
                    "total": total_futures,
                },
            )
        ...
        for future in tqdm(
            ...
            try:
                paths, points = future.result()  # Output in CRS 4326
                ...
                if task_celery:
                    completed = len(final_paths)
                    task_celery.update_state(
                        state="PROGRESS",
                        meta={
                            "status": f"Completed {completed}/{total_futures} clusters for area {area}",
                            "total": total_futures,
                            "completed": completed,
                        },
                    )
...
# ===============
def unsupervised_ring(
    sites_input: gpd.GeoDataFrame,
    hubs_input: gpd.GeoDataFrame,
    area: str,
    member_expectation: int = 10,
    member_tolerance: int = 0,
    area_col="region",
    export_dir=None,
    ...
    **kwargs,
    ...
    print(f"\n🧩 Ring Network Unsupervised | {area}")
    ...
    try:
        ...
        if task_celery:
            task_celery.update_state(
                state="PROGRESS",
                meta={
                    "status": f"Clustering sites for area {area}",
                },
            )

        # Node Sindex
        node_sindex = nodes.sindex
        sites_input['nearest_node'] = sites_input.geometry.apply(lambda geom: nodes.at[node_sindex.nearest(geom)[1][0], 'node_id'])
        sites_input = sites_input.reset_index(drop=True)
        print(f"ℹ️ Total Sites | {area_col} {area}: {len(sites_input):,}")

        # CLUSTERING
        cluster_path = os.path.join(export_dir, f"Cluster_Sites_{area}.parquet")
        if os.path.exists(cluster_path):
            print(f"ℹ️ Cluster file already exists for {area_col} {area}. Loading existing clusters...")
            clustered_gdf = gpd.read_parquet(cluster_path)
            if 'ring_name' not in clustered_gdf.columns:
                print("⚠️ No cluster column found in existing data. Re-clustering...")
            else:
                print(f"✅ Clusters loaded successfully for {area_col} {area}.")
        else:
            print(f"ℹ️ No existing cluster file found for {area_col} {area}. Running clustering...")
            clustered_gdf = parallel_finding_hubs(sites_input, hubs_input, roads, nodes, member_expectation=member_expectation, member_tolerance=member_tolerance, drop_existings=drop_existings, max_distance=max_distance)
            clustered_gdf.to_parquet(os.path.join(export_dir, f"Cluster_Sites_{area}.parquet"), index=False)

        unique_clusters = clustered_gdf["ring_name"].dropna().unique().tolist()
...
# DATA EXPORT
# ===============
def merge_kmz(main_kml, ring_data, point_data, folder, separator: str = "-"):
    ...
    ring_route["route_name"] = ring_route["near_end"] + separator + ring_route["far_end"]

    # SITE LIST & HUB
    ring_sites = cluster_points[cluster_points["site_type"] != "FO Hub"].copy()
    ring_hub = cluster_points[cluster_points["site_type"] == "FO Hub"].copy()
...
def merge_gpkg(gpkg_path, ring_data, point_data, cluster):
    ...
    ring_sites = ring_sites[ring_sites["site_type"] != "FO Hub"].copy()
    if ring_sites.empty:
...
def nearfar_connection(points_gdf: gpd.GeoDataFrame, paths_gdf: gpd.GeoDataFrame):
    miss_point = [col for col in ["site_id", "ring_name"] if col not in points_gdf.columns]
    miss_paths = [
        col
        for col in ["near_end", "far_end", "ring_name"]
        if col not in paths_gdf.columns
    ]
...
# CLUSTERING
# ================
def parallel_finding_hubs(sites_gdf: gpd.GeoDataFrame, hubs_gdf: gpd.GeoDataFrame, roads: gpd.GeoDataFrame, nodes: gpd.GeoDataFrame, **kwargs):    
    print(f"🧩 Clustering and Finding Nearest Hubs")
    member_expectation = kwargs.get('member_expectation', 10)
    max_distance = kwargs.get('max_distance', 10000)
    ...
    identified_hubs = main_clustering(G, sites_gdf, member_expectation=member_sitelist, tolerance=member_tolerance, max_distance=max_distance, hubs_gdf=hubs_gdf, drop_existings=drop_existings)
...
def main_unsupervised(
    ...
    **kwargs,
    ...
    if os.path.exists(output_paths) and os.path.exists(output_point):
        ...
        print(f"\n🔥 Starting Fiberization Process | Method: {method.title()}")
        print(f"ℹ️ Member Expectation: {member_expectation}")
        print(f"ℹ️ Member Tolerance: {member_tolerance}")

        if task_celery:
            task_celery.update_state(state="PROGRESS", meta={"current": 0, "total": len(area_list), "status": f"Starting Fiberization Process | Method: {method.title()}"})
        
        all_paths = []
        all_points = []
        hubs_data['site_type'] = 'FO Hub'
        for area in tqdm(area_list, desc=f"Processing {area_col}"):
            site_area = site_data[site_data[area_col] == area].copy()
            hubs_area = hubs_data[hubs_data[area_col] == area].copy()

            if site_area.empty:
                print(f"⚠️ No data found for {area_col} {area}. Skipping...")
                continue

            result = unsupervised_ring(
                sites_input=site_area,
                hubs_input=hubs_area,
                area=area,
                area_col=area_col,
    ...
    # EXPORT EXCEL
    with pd.ExcelWriter(
        os.path.join(export_loc, f"Ring_Network_{method.title()}.xlsx"),
        engine="openpyxl",
    ) as writer:
    ...
    for area in area_list:
        ...
        try:
            ...
            kmz_main = merge_kmz(kmz_main, ring_data, point_data, folder=folder, separator=";")
...
if __name__ == "__main__":
    excel_file = r"D:\JACOBS\TASK\OKTOBER\Week 2\SURGE BACKHAUL UNSUPERVISED\Surge_Site POP Sitelist 5800_13102025_v2.xlsx"
    export_dir = r"D:\JACOBS\TASK\OKTOBER\Week 2\SURGE BACKHAUL UNSUPERVISED"
    area_col = 'region'
    cluster_col = 'ring_name'
    member_expectation = 12
    member_tolerance = 0
    ...
    try:
        with pd.ExcelFile(excel_file) as xls:
            ...
            nr_sites.to_parquet(os.path.join(export_dir, "Debug New Ring Sites.parquet"))
            nr_hubs.to_parquet(os.path.join(export_dir, "Debug New Ring Hubs.parquet"))
            
            if 'region' not in nr_hubs.columns:
                print(f"ℹ️ Generate Region")
                group = auto_group(nr_hubs, distance=max_distance)
                group.to_parquet(os.path.join(export_dir, "Groups.parquet"))
                
                group = group.to_crs(epsg=3857)
                nr_sites = nr_sites.to_crs(epsg=3857)
                nr_hubs = nr_hubs.to_crs(epsg=3857)

                nr_sites = gpd.sjoin(nr_sites, group[['geometry', 'region']]).drop(columns='index_right')
                nr_hubs = gpd.sjoin(nr_hubs, group[['geometry', 'region']]).drop(columns='index_right')
    ...
    date_today = datetime.now().strftime("%Y%m%d")
    export_loc = f"{export_dir}/New_Ring/Unsupervised/{date_today}"
