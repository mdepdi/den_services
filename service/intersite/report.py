...
def boq_generation(
    ...
    for ring_name, gdf_ring_route in gdf_route.groupby("ring_name"):
        ...
        for idx, seg_row in gdf_ring_segments.iterrows():
            ...
            # ---------------------------
            fo_factor = 1 + (cable_percentage/100)
            calc_permission_pu = max(math.floor(len_bb_m + len_access_m - len_pole_m + sum(len_cable_by_core_m.get(core, 0) for core in len_cable_by_core_m.keys() if int(core) != 24)), 0)
